...
export const GET = async(req:NextRequest) => {
    ...
    const business_number = sp?.get("business_number") || '';
    const queryData = {business_number: business_number, };

    const api = await ArcServerApi.init();
    const rs = await api.GET$Auth$duplicateBusinessNumber( queryData);
