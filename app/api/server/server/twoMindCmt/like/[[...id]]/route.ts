...
export const GET = async(req:NextRequest, params:{id:any}) => {
    const id = params?.id?.length > 0 ? params?.id[0] : ''
    const url = new URL(req.url);
    const sp = url?.searchParams;
    const ip_address = sp?.get("ip_address") || '';
    const queryData = {ip_address: ip_address, };

    const api = await ArcServerApi.init();
    const rs = await api.GET$TwoMindCmt$like(id, queryData);
